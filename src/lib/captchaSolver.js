...
class NextCaptchaProvider extends CaptchaSolverProvider {
	...
	async solve(base64Image) {
		...
		const createRes = await fetch('https://api.nextcaptcha.com/createTask', {
			method: 'POST',
		...
		console.log(`[${this.name}] createTask response:`, JSON.stringify(createData));

		// Handle errors from the API
		if (createData.errorId && createData.errorId !== 0) {
			throw new Error(`NextCaptcha error: ${createData.errorDescription || createData.errorCode || 'Unknown'}`);
		}

		// Some APIs return the solution immediately
		if (createData.solution?.text) {
			console.log(`[${this.name}] Solved immediately: "${createData.solution.text}"`);
			return createData.solution.text;
		}
		if (createData.status === 'ready' && createData.solution) {
			const text = createData.solution.text || createData.solution.gRecaptchaResponse || '';
		...
		if (!taskId) {
			throw new Error(`NextCaptcha: No taskId in response: ${JSON.stringify(createData)}`);
		}
		...
		while (attempts < 30) {
			...
			const resultRes = await fetch('https://api.nextcaptcha.com/getTaskResult', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
			...
			if (result.status === 'failed') {
				throw new Error(`NextCaptcha task failed: ${result.errorDescription || 'Unknown'}`);
			}
			attempts++;
		}
...
class FreeCaptchaBypassProvider extends CaptchaSolverProvider {
	...
	async solve(base64Image) {
		...
		const createRes = await fetch('https://freecaptchabypass.com/createTask', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				clientKey: this.apiKey,
		...
		if (createData.errorId && createData.errorId !== 0) {
			throw new Error(`FreeCaptchaBypass error: ${createData.errorDescription || createData.errorCode || 'Unknown'}`);
		}
		...
		if (!taskId) {
			throw new Error(`FreeCaptchaBypass: No taskId in response: ${JSON.stringify(createData)}`);
		}
