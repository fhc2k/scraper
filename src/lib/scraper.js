...
export async function scrapeCEP(queryData, captchaConfig = {}) {
    ...
    try {
        ...
        console.log(
            `CAPTCHA Provider: ${solver.name} ${isAutoMode ? "🤖" : "👤"}`,
        );
        ...
        await page.type(
            "#input_cuenta",
            queryData.cuentaBeneficiaria.replace(/\s/g, ""),
        );
        await page.type(
            "#input_monto",
            queryData.monto.toString().replace(/,/g, ""),
        );
        ...
        await page.evaluate(() => {
            ...
            const captchaInputDiv =
                document.querySelector("#input_captcha")?.parentElement;
            if (captchaInputDiv) {
        ...
        if (isAutoMode) {
            ...
            if (!solution) {
                return {
                    success: false,
                    error: `${solver.name}: No pudo resolver el CAPTCHA`,
                };
            }
        ...
        const outcome = await Promise.race([
            page
                .waitForSelector(".msg-banxico .boton-descarga-xml", {
                    timeout: 45000,
                })
                .then(() => "success"),
            page
                .waitForSelector("#consultaMISPEI .info", {
                    visible: true,
                    timeout: 45000,
                })
                .then(() => "not_found"),
            page
                .waitForSelector(
                    ".mensajeError, .error, .alert-danger, #mensajeError",
                    { visible: true, timeout: 45000 },
                )
                .then(() => "error"),
            page
                .waitForFunction(
                    () => {
                        ...
                        return (
                            dialogEl &&
                            dialogEl.innerText &&
                            dialogEl.innerText.toLowerCase().includes("error")
                        );
                    },
                    { timeout: 45000 },
                )
        ...
        if (outcome === "js_alert") {
            return {
                ...
                error:
                    dialogMessage ||
                    "La información consultada no existe o el CAPTCHA es incorrecto.",
            };
        }
        ...
        if (outcome === "error" || outcome === "dialog_error") {
            const errorMsg = await page.evaluate(() => {
                ...
                const dialog = document.querySelector(
                    '.ui-dialog:not([style*="display: none"]) .ui-dialog-content',
                );
        ...
        if (outcome === "timeout") {
            ...
            if (
                ...
                return {
                    success: false,
                    error: "No se encontró información del CEP.",
                };
            }
            return {
                success: false,
                error: "Tiempo de espera agotado. Verifica el CAPTCHA o los datos.",
            };
        }

        // ── Step 7: Final Data Extraction via XML ──
        console.log(
            "Success popup detected! Fetching and parsing XML data directly...",
        );
        ...
        const extraction = await page.evaluate(async () => {
            try {
                ...
                const cleanMonto = rawMonto
                    ? parseFloat(rawMonto).toString()
                    : "";

                return {
                    data: {
                        id_transaccion:
                            getAttr("SPEI_Tercero", "claveRastreo") || "N/A", // Using claveRastreo as ID conceptually
                        estado: "LIQUIDADO", // If there's an XML, it's liquidado
                        fecha_operacion: getAttr(
                            "SPEI_Tercero",
                            "FechaOperacion",
                        ),
        ...
        if (extraction.error || !extraction.data) {
            return {
                success: false,
                error: `Error extrayendo datos XML: ${extraction.error || "Desconocido"}`,
            };
        }
