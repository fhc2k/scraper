...
import { Calendar, Hash, Building2, User, CreditCard, DollarSign, ChevronRight, AlertCircle, ChevronDown, Key, Bot, Settings2 } from 'lucide-react';
...
const CEPForm = ({ onSubmit }) => {
  ...
  const InputField = ({ label, name, icon: Icon, type = 'text', placeholder, children }) => (
    <div className="relative mb-6">
      <label className="block text-sm font-medium text-gray-400 mb-1 ml-1" htmlFor={name}>
        {label}
      </label>
      <div className="relative group">
        <div className={cn(
          "absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors z-10",
          errors[name] ? "text-red-500" : "group-focus-within:text-blue-500 text-gray-500"
        )}>
          <Icon size={18} />
        </div>
        {children || (
  ...
  return (
    <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 lg:p-8 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-6 sm:mb-8 pb-4 sm:pb-6 border-b border-white/5">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-white tracking-tight">Nueva Consulta de Pago</h2>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Completa los campos para validar el CEP</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1">
          ...
          <div className="md:col-span-1">
            ...
            >
              ...
              >
                <option value="" className="bg-[#0a0a0a]">Seleccionar banco...</option>
                {MEXICAN_BANKS.map(bank => (
                  <option key={bank.id} value={bank.id} className="bg-[#0a0a0a]">
                    {bank.name}
                  </option>
                ))}
              </select>
